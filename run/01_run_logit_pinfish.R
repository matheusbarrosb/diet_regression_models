...
df =
ggmcmc::ggs(fit, family = "p_status")
...
## 2. beta_status plot ##
post = rstan::extract(fit, pars = "beta_status")$beta_status

df =
ggmcmc::ggs(fit, family = "beta_status") %>%
  mutate(
    # Extract indices from Parameter: beta_status[prey,status]
    indices = str_extract(Parameter, "\\[(.*?)\\]"),
    indices = str_remove_all(indices, "\\[|\\]"),
    prey_idx   = as.integer(str_split_fixed(indices, ",", 2)[,1]),
    status_idx = as.integer(str_split_fixed(indices, ",", 2)[,2]),
...
post = rstan::extract(fit, pars = "beta_TL")$beta_TL
df =
ggmcmc::ggs(fit, family = "beta_TL") %>%
  mutate(
...
ggarrange(
  ...
  align = "h",
  nrow = 1,
  widths = c(1.14, 0.8, 0.8)
)

ggsave(
  filename = here::here("res", "figures", "pinfish_probs.pdf"),
  width = 7,
  height = 3
)
...
post_pred <- rstan::extract(fit, pars = "p_site")$p_site 
...
ggplot(aes(x = Observed, y = Model_Mean)) +
  geom_point(size = 1) +
